...
public:
	...
	Bot* clone(){
		Ogre* o = new Ogre();
		o->setCurrentState(getCurrentState());
		o->setSpriteType(getSpriteType());
		return o;
	}

	b2Fixture* getFix(){	return fix;	}
	...
	static void registerLua(lua_State *state){
		...
		module(state)[
			class_<Ogre>("Ogre")
				.def("setCurrentState",&Ogre::setCurrentStateA)
				.def("setSpriteType",&Ogre::setSpriteType)
		];
	}
};