...
FractureObject *sphere_fracture = NULL;

bool TestScene::onInitialize()
{
	//-----------------------------------------------------------------
	...
	m_graph->pushNode();
	{
        ...
        //// create a conveyor
		Conveyor *conveyor1 = new Conveyor(this, Vector3(10.0f, 1.0f, 40.0f), 2.5f );
		...
		conveyor1->setPosition(Vector3(45.0f, 0.0f, -40.0f));
		conveyor1->setYaw(90.0f);
		conveyor1->setPitch(0.0);
    ...
    //-----------------------------------------------------------------
	// create a circular platform with an explodable globe in the center
	//-----------------------------------------------------------------
    m_graph->pushNode();
    {
        /*OBJmodel *circle_platform = new OBJmodel(this, "Objects/circle_platform.obj", 1.0f, false, false ,true);
        if ( !circle_platform->initialize(new LightShader(0.2f, 1.0, 8, RAND_COLOR))) { GLOG("ERROR: could not create OBJmodel\n"); return false; }
        circle_platform->setPosition(Vector3(60.0f, 0.0f, 0.0f));
        circle_platform->addToDynamicWorld(m_dynamicsWorld);
        m_graph->attach(circle_platform);*/
        
        //// create a platform
		Platform *platform = new Platform(this, Vector3(70.0f, 2.0f, 70.0f));
		if ( !platform->initialize(new LightShader(0.1f, 1.0f, 4, RAND_COLOR))) { GLOG("ERROR: Could not initialize center platform\n"); return false; }
        platform->setPosition(Vector3(60.0f, 0.0f, 0.0f));
		platform->addToDynamicWorld(m_dynamicsWorld);
		m_graph->attach(platform);
        
        // create a fracture sphere that explodes
        Sphere *sphere = new Sphere(this, 20.0f);
        if ( !sphere->initialize(new LightTexScrollShader(0.9f, 0.2f, 2, Color(1.0f, 1.0f, 1.0f, 1.0f), Vector3(1.0f, 0.0f, 0.0f), 0.001f))) { GLOG("ERROR: could not initialize sphere.\n"); return false; }
        sphere->setPosition(Vector3(60.0f, 18.0f, 0.0f));
        sphere->setOrientation(0.0f, -90.0f, 0.0f);
        sphere->loadTexture("Textures/earth.tga");
        sphere_fracture = new FractureObject(sphere, "Textures/voronoi_fracure.tga", 40.0f);
    ...
    // create a particle system
    m_particleSystem = new StarryParticleSystem(2000);
    m_particleSystem->startSystem();
    m_graph->attach(m_particleSystem);
